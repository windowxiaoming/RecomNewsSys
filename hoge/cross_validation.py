...
def user_based_train_test_split(interactions,
    ...
    seed = random_state.randint(minint, maxint, dtype=np.int32)
    in_test = ((murmurhash3_32(np.array(interactions.user_ids,dtype=np.int32),seed=seed,positive=True) % 100 / 100.0) < test_percentage)
